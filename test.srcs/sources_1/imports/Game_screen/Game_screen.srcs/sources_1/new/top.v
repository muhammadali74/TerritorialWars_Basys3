...
//    pixel_gen_torture pg1(.clk(clk_100MHz), .clk_25(clk_d2), .reset1(reset1), .reset2(reset2), .random(random), .up1(up1),.down1(down1),.right1(right1),.left1(left1),.up2(up2),.down2(down2),.right2(right2),.left2(left2), .video_on(w_video_on), 
//                        .x(w_x), .y(w_y), .rgb(rgb_next));
